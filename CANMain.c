...
void ConfigCAN(void) {
    ...
    PIR5bits.RXB1IF = 0;
    PIE5bits.RXB1IE = 1; //Enable RXB0 interrupt
}
...
void __interrupt(high_priority) HighISR(void) {
    if (INTCONbits.INT0IF) {
        ...
        //This code is for any message received in RXB0
        ...
        //This code is for any message received in RXB1
        id = RXB1SIDH; //Get upper 8 bits of ID
        id <<= 3; // and shift over to make room for lower 3 bits
        id |= RXB1SIDLbits.SID; //get lower 3 bits
        len = RXB1DLCbits.DLC; //Get length - should be 2 since we're sending ints
        data = RXB1D1; //high byte of data
        data <<= 8;
        data += RXB1D0; //low byte of data
        update = 1; //Set update so the main loop knows we have a new message
        RXB1CONbits.RXFUL = 0; //Clear the buffer - we're done with it
        PIR5bits.RXB1IF = 0; //Clear the interrupt flag
    }
}
